...
static struct Payload {
    ...
    uint16_t turretDest;    //The following five are for the arm/Science module
    uint16_t shoulderDest;
    uint16_t elbowDest;
    uint16_t forearmDest;
    uint16_t plateshiftDest;
    uint8_t handDest;       //I am not sure what we use this for... -Taylor Welker
} Payload;

// Arm/Science positions
...
volatile uint16_t turretPos;    //Only used in the Arm Module
volatile uint16_t shoulderPos;  //Connected to the science plate for science
volatile uint16_t elbowPos;     //Connected to the science drill for science
volatile uint16_t forearmPos;   //Connected to the Science Elevator for science
volatile uint16_t plateshiftPos; //Only used in the Science Module 
...
#define POSITION_PAYLOAD_SIZE (15) // 1 start byte, 2 bytes per joint, 5 joints, 4 bytes of science data
...
// [preamble?, turretlo, turrethi, shoulderlo, shoulderhi, elbowlo, elbowhi,
// forearmlo, forearmhi, plateshiftlo, plateshifthi, temperaturelo, temperaturehi, humiditylo, 
// humidityhi]
...
static enum compRxStates_e { pre0, leftlo, lefthi, leftdir, rightlo, 
    ...
    elbowlo, elbowhi, forearmlo, forearmhi, plateshiftlo, plateshifthi,
    handlo, chutes} compRxState;
...
int compRxEventHandler() {
          ...
          (UART_Computer_ReadRxStatus() & UART_Computer_RX_STS_FIFO_NOTEMPTY))
    {
        ...
        case forearmhi:
            ...
            compRxState = plateshiftlo; // change state
            break;
        case plateshiftlo:
            Payload.plateshiftDest = byte;
            compRxState = plateshifthi; // change state
            break;
        case plateshifthi:
            Payload.plateshiftDest |= byte << 8;
            pololuControl_driveMotor(Payload.plateshiftDest,
                POLOLUCONTROL_PLATESHIFT);
...
// Update forearm position
void updatePlateShiftPos() {
	static enum states_e {low, high} state = low;
    
    static uint16_t temp;
    while(UART_PlateShift_ReadRxStatus() & UART_PlateShift_RX_STS_FIFO_NOTEMPTY) {
        switch(state) {
            case low:
                temp = UART_PlateShift_GetByte() & 0xff;
                state = high;
            break;
            case high:
                if (temp <= 4095) {
                    temp |= (UART_PlateShift_GetByte() << 8) & 0xff00;
                }
                plateshiftPos = temp;
                state = low;
            break;
...
static void feedbackToOnboardComputer() {
    ...
    feedbackArray[9] = (plateshiftPos & 0xff);
    feedbackArray[10] = ((plateshiftPos >> 8) & 0xff);
	feedbackArray[11] = ((temperature & 0xff));
	feedbackArray[12] = ((temperature >> 8) & 0xff);
	feedbackArray[13] =((humidity & 0xff));
	feedbackArray[14] =((humidity >> 8) & 0xff);
...
static void feedbackToTerminal() {
    ...
    char pos[34];
    sprintf(pos, "\n\r\n\rpositions:%4d,%4d,%4d,%4d, %4d",
        turretPos, shoulderPos, elbowPos, forearmPos, plateshiftPos);
